...
async function tryDynamicAnswer(filePath, response, request, RELOAD) {
  ...
  if( ! fileExists(filePath + '.js') && ! fileExists(filePath + '.sse')) return


  // We want to load our dynamic file with require later, unlike with js-files
  // it won't do it without the extension:

  if(fileExists(filePath + '.sse')) filePath += '.sse'
  ...
  // An sse-file is an answer by itself:

  if(filePath.endsWith('.sse')) return true
