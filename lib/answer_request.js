...
function genNoAnswerWarning(filePath) {
  ...
  return `
In ${__filename} executing the function loaded from ${filePath} returned
undefined, which leads to a 404-not-found response and may not be what
you want. If this is intended, let the dynamic file return null, to let
...
async function tryDynamicAnswer(filePath, response, request) {
  ...
  try {
    ...
    console.error(genCouldNotLoadError(filePath), error)
  ...
  if(typeof(dynamicAnswer) == 'function') {
    ...
    try {
      ...
      console.error(genCouldNotExecuteError(filePath), error)
...
*/
  ...
  // Ignore dynamicAnswer is undefined (results in a 404 not-found):

  if(dynamicAnswer !== null) {

    console.log(genNoAnswerWarning(filePath))
